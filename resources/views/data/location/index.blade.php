...
<script>
    ...
    {
        var ids = jQuery("#locationGrid").jqGrid('getDataIDs'),
            edt = '',
            del = ''; 
        for(var i=0;i < ids.length;i++){ 
            var cl = ids[i];
            var rowdata = $('#locationGrid').getRowData(cl);
            var dataid = rowdata.location_id;

            edt = '<a href="{{ route("location-edit",'') }}/'+dataid+'"><i class="fa fa-pencil"></i></a> ';
            del = '<a href="{{ route("location-delete",'') }}/'+dataid+'" onclick="if (confirm(\'Are You Sure ?\')){return true; }else{return false; };"><i class="fa fa-close"></i></a>';
            jQuery("#locationGrid").jqGrid('setRowData',ids[i],{action:edt+' '+del}); 
        } 
    }
</script>
...
<!--        <div class="box-tools">
            <a href="{{ route('location-create') }}" type="button" class="btn btn-block btn-info btn-sm"><i class="fa fa-plus"></i> Add New</a>
        </div>-->
    </div>
    <div class="box-body table-responsive">
            {{
                GridRender::setGridId("locationGrid")
                ->enableFilterToolbar()
                ->setGridOption('url', URL::to('/location/grid-data'))
                ->setGridOption('editurl',URL::to('/location/crud'))
